...
import FounderGreetingAudio from "@/components/FounderGreetingAudio";
...
export default function Home() {
  ...
  return <main>
    <section className="home-hero">
      <div className="hero-copy"><Reveal><p className="eyebrow">FINE ART • SAHARANPUR • EST. 2017</p><h1>Where passion<br/>becomes <span className="hero-em-wrap"><em>art.</em><svg className="hero-flourish" viewBox="0 0 120 20" aria-hidden="true" focusable="false"><path d="M4 14 Q30 4 62 10 T116 6" /></svg></span></h1><p className="hero-sub">A trusted women-only academy and fine-art studio creating confident artists and timeless handmade paintings.</p><div className="hero-actions"><InquiryLink ref={heroCtaRef} message="Hello Meenakshi Art Work, I would like details about academy enrollment.">Join Academy</InquiryLink><HeroAudioPlayer /></div></Reveal></div>
    ...
    <section className="services section-wrap"><Reveal className="section-heading"><div><p className="eyebrow">WHAT WE CREATE</p><h2>Art for your hands,<br/>home and <em>heart.</em></h2></div><p>Whether you want to learn, collect or commission, every experience is guided personally.</p></Reveal><div className="services-grid">{services.map(([n, title, text, href, image, cta], i) => <Reveal key={title} className="service-card" delay={i * .08}><ServiceCardLink href={href}><div className="service-image"><div className="service-image-bg" style={{ backgroundImage: `url('${image}')` }}></div><span>{n}</span></div><h3>{title}</h3><p>{text}</p><b>{cta} →</b></ServiceCardLink></Reveal>)}</div></section>
    ...
    <section className="founder-section" id="meet-the-artist"><div className="founder-portrait"><div className="portrait-image"><Image
      ...
      priority
    /></div><p>Meenakshi · Founder & Lead Instructor</p></div><Reveal className="founder-copy"><p className="eyebrow">MEET THE ARTIST</p><FounderGreetingAudio /><h2>The heart behind<br/><em>every brushstroke.</em></h2><p>Founded in 2017, Meenakshi Oil Painting was built with a vision to teach fine art in a secure, disciplined environment—while creating premium handcrafted artwork for people who value originality.</p><blockquote><p className="founder-quote-text">&ldquo;{founderQuote}&rdquo;</p><div className="founder-quote-signature"><svg viewBox="0 0 200 80" className="founder-signature-svg"><text x="10" y="60" className="founder-signature-name">Meenakshi</text><path className="founder-signature-flourish" d="M10,68 C60,78 120,78 175,66 C185,64 190,68 182,72" /></svg><FounderGreetingAudio /></div></blockquote><button type="button" className="founder-quote-more" onClick={showAnotherNote}>Another note <span>↻</span></button><Link href="/journal" className="line-link">Our story & studio <span>→</span></Link></Reveal></section>
